...
============
*/
int indexAllFunctions = 0;
int declareAllFunctions = 1;
void PR_PrintSymbols (int type)
{
	def_t	*d;
	// first generate string definitions and values
	int	 l, j;
    char *str;
    int n_symbol = 1; // first is null string
    if (type & PRINT_SYMBOL_INDEX)
    {
        qc_printf("#define NULL_string_index      0\r\n");
    }
	for (d = pr.def_head.next ; d ; d = d->next)
	{
	    if (d->type->type == ev_string && d->initialized)
        {
            str = G_STRING(d->ofs);
        }
        else if (d->type->type == ev_field)
        {
            str = d->name;
        }
        else
            continue;
		l = strlen(str); // strlen(strings+i) + 1;
		char symbolized[MAX_TEMP_EXPRESSION_TYPE] = {0};
		symbolizeString(symbolized, str);
		int nspaces = MAX_STRING_SYMBOL_LENGTH + 32 - strlen(symbolized);
        if (type & PRINT_SYMBOL_INDEX)
        {
		    if (strcmp("NULL_string_index", symbolized))    // don't write NULL_string_index
		        qc_printf("#define %s %*s %d\r\n", symbolized, nspaces, "", n_symbol);
        }
        n_symbol++;
	}
    char symbolized[MAX_TEMP_EXPRESSION_TYPE] = {0};
	// last string is for temp string for print operation
    symbolizeString(symbolized, "tempPrintString");
    int nspaces = MAX_STRING_SYMBOL_LENGTH + 32 - strlen(symbolized);
    if (type & PRINT_SYMBOL_INDEX)
    {
        qc_printf("#define %s %*s %d\r\n", symbolized, nspaces, "", n_symbol);
    ...
    if (type & PRINT_SYMBOL_ARRAY)
    {
    	qc_printf("const char *const quakeProgStrings[%d] = \r\n{\r\n", n_symbol);
        qc_printf("    \"%s\",\r\n", "");
    }
	for (d = pr.def_head.next ; d ; d = d->next)	{
	    if (d->type->type == ev_string && d->initialized)
        ...
        else if (d->type->type == ev_field)
        {
            ...
            continue;
		l = strlen(str); // strlen(strings+i) + 1;
		char s[MAX_TEMP_EXPRESSION_TYPE] = {0};
		int p = 0;
		for (j=0 ; j < l ; j++)
		{
			if (str[j] == '\n')
    ...
    qc_printf("#ifdef QUAKE_GAME\r\n");
    // functions. Number of symbol starts from 1, because 0 is null pointer
    nspaces = MAX_STRING_SYMBOL_LENGTH + 32 - strlen("NULL");
    if (type & PRINT_SYMBOL_INDEX)
    {
        qc_printf ("#define %s_func_index %*s %d\r\n", "NULL", nspaces, "", 0);
    }
    n_symbol = 1;
    for (d = pr.def_head.next ; d ; d = d->next)
	{
	    if (d->type->type == ev_function && (d->functionIndexUsed || indexAllFunctions) && d->isRealFunction)
        {
            str = d->name;
        }
        else
            continue;
		int nspaces = MAX_STRING_SYMBOL_LENGTH + 32 - strlen(str);
        if (type & PRINT_SYMBOL_INDEX)
        {
    		qc_printf ("#define %s_func_index %*s %d\r\n", str, nspaces, "", n_symbol);
        }
        n_symbol++;
	}
	...
	for (d = pr.def_head.next ; d ; d = d->next)
	{
	    if (d->type->type == ev_function && (d->functionIndexUsed || declareAllFunctions) && d->isRealFunction)
        {
            str = d->name;
        }
        else
        ...
        getParamTypeName(paramTypeName, d->type->aux_type, "", 0);
        if (type & PRINT_SYMBOL_INDEX)
        {
            qc_printf("%s%s(", paramTypeName, str);
            for (int i=0 ; i<d->type->num_parms ; i++)
            {
                if (i >= 1)
                    qc_printf(", ");
                getParamTypeName(paramTypeName, d->type->parm_types[i], "", 0);
                qc_printf("%s", paramTypeName );
            }
            if (d->type->num_parms == 0)
            {
                qc_printf("void");
            }
            else if (d->type->num_parms == -1)
            {
                qc_printf("...");
            }
            qc_printf(");\r\n");
    ...
    if (type & PRINT_SYMBOL_ARRAY)
    {
        qc_printf("const void *const quakeProgFunctions[%d] = \r\n{\r\n", n_symbol);
        qc_printf("    NULL,\r\n");
    }
    for (d = pr.def_head.next ; d ; d = d->next)
	{
    ...
    qc_printf("#endif // #ifdef QUAKE_GAME\r\n");
...
============
*/
char * strendswith(char *str, char *suffix)
{
    ...
    if (suffl > strl)
      ...
      return strstr(&str[strl-suffl], suffix);
}
void	PR_WriteNewProgdefs (char *filename)
{
	def_t	*d;
	FILE	*f;
...
// print global vars until the first field is defined
	...
	for (d=pr.def_head.next ; d ; d=d->next)
	{
		if (!strcmp (d->name, "end_sys_globals"))
			continue;
		if (!strcmp (d->name, "qcc_end_sys_fields"))
			continue;
        if (d->scope)
            continue;
        if (d->initialized)
		...
		case ev_float:
		    // skip _x, _y _z
		    if (strendswith(d->name, "_x") || strendswith(d->name, "_y") || strendswith(d->name, "_z"))
            {
		        break;
            }
			fprintf (f, "\tfloat\t%s;\n",d->name);
	...
	fprintf (f,"} progsvars_t;\n\n");
    fprintf(f, "extern progsvars_t %s;\r\n", GLOBAL_STRUCT_NAME);
