...
{
    KeyUp(&in_mlook);
    if (!(in_mlook.state & 1) && lookspring)
        V_StartPitchDrift();
}
void IN_UpDown(void)
{
...
void CL_AdjustAngles(void)
{
    ...
    {
        _g->cl.viewangles[YAW] -= speed * cl_yawspeed * CL_KeyState(&in_right);
        _g->cl.viewangles[YAW] += speed * cl_yawspeed * CL_KeyState(&in_left);
        _g->cl.viewangles[YAW] = anglemod(_g->cl.viewangles[YAW]);
    }
    if (in_klook.state & 1)
    {
        V_StopPitchDrift();
        _g->cl.viewangles[PITCH] -= speed * cl_pitchspeed * CL_KeyState(&in_forward);
        _g->cl.viewangles[PITCH] += speed * cl_pitchspeed * CL_KeyState(&in_back);
    }

    up = CL_KeyState(&in_lookup);
    down = CL_KeyState(&in_lookdown);

    _g->cl.viewangles[PITCH] -= speed * cl_pitchspeed * up;
    _g->cl.viewangles[PITCH] += speed * cl_pitchspeed * down;
    ...
    if (_g->cl.viewangles[ROLL] > 50)
        _g->cl.viewangles[ROLL] = 50;
    if (_g->cl.viewangles[ROLL] < -50)
        _g->cl.viewangles[ROLL] = -50;
...
//
    ...
    MSG_WriteFloat(&buf, _g->cl.mtime[0]);	// so server can get ping times

    for (i = 0; i < 3; i++)
        MSG_WriteAngle(&buf, _g->cl.viewangles[i]);
...
// from the last level
//
    if (++_g->cl.movemessages <= 2)
        return;

    if (NET_SendUnreliableMessage(_g->cls.netcon, &buf) == -1)
    {
